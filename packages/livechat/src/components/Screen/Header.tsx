...
const ScreenHeader = ({
	...
	return (
		...
		>
			{agent?.avatar && (
				<Header.Picture>
					<Avatar src={agent.avatar.src} description={agent.name ?? agent.avatar.description} status={agent.status} large={largeHeader()} t={t} />
				</Header.Picture>
			)}
			...
			<Tooltip.Container>
				<Header.Actions>
					<Tooltip.Trigger content={notificationsEnabled == false ? t('sound_is_on') : t('sound_is_off')} placement='bottom-left'>
						<Header.Action
							aria-label={notificationsEnabled == false ? t('sound_is_on') : t('sound_is_off')}
