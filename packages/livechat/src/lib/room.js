...
const displaySessionEndingAlert = async (timeout) => {
	...
	displaySessionEndingAlertTimeout = setTimeout(async () => {
		const alert = { id: livechatSessionEndingAlertId, children: i18next.t('session_ending_at', { time: dateEnd.toLocaleTimeString() }), warning: true, timeout: timeoutMs, hideCloseButton: true, sessionExtension: true }
		const { alerts } = store.state;
		await store.setState({ alerts: (alerts.push(alert), alerts) });
	}, alertTomeut)
};
